...
export class ProvinceStatsRepository {
    // โหลดไฟล์ข้อมูลจังหวัดครั้งเดียวตอนสร้าง repository
    constructor(path) {
        this.items = JSON.parse(readFileSync(path, 'utf-8'));
        this.defaultYear = Math.max(...this.items.map(item => item.year));
    }

    // ค้นหาข้อมูลจังหวัด ถ้าระบุปีจะคืน record เดียว ถ้าไม่ระบุปีจะคืนหลายปี
    ...
    // ดึงปีทั้งหมดที่มีข้อมูลของจังหวัดนั้น เพื่อนำไปสร้างปุ่มเลือกปี
    yearsByProvince(name) {
        const records = this.findByProvince(name);
        return records ? records.map(item => item.year) : [];
    }

    // ค้นข้อมูลหลายจังหวัดตาม field, ช่วงค่า min/max และปีที่ต้องการ
...
// แปลง record จาก JSON ให้เป็นข้อความภาษาไทยสำหรับตอบกลับผู้ใช้
...
// อ่านข้อความรูปแบบ "จังหวัด ปี" เช่น "เชียงราย 2026"
...
// ตัดคำสั่งนำหน้าออก เช่น "สรุปข้อมูล เชียงราย" จะได้ "เชียงราย"
function afterPrefix(text, prefix) {
    return text.startsWith(`${prefix} `) ? text.slice(prefix.length).trim() : null;
}

// สร้าง URL รูปกราฟจาก QuickChart แล้วส่งกลับเป็น image message ของ LINE
...
// สร้างปุ่มใน Flex Message โดย action จะส่งข้อความกลับเข้าบอต
...
// เมนูหลัก 3 feature ให้สอดคล้องกับ Rich Menu
...
export class LineService {
    // รวม dependency ที่บอตต้องใช้ ได้แก่ LINE client, OpenAI client และ repository ข้อมูลจังหวัด
    ...
    // เลือก flow ตามข้อความที่ผู้ใช้ส่งมา เช่น เมนู, กราฟ, กรองข้อมูล หรือ AI
    async respond(input) {
    ...
    // สร้างกราฟย้อนหลังของจังหวัดที่ระบุ
    chart(province) {
    ...
    // สร้างปุ่มปีทั้งหมดของจังหวัดที่ผู้ใช้เลือก
    yearPicker(province) {
    ...
    // ตอบข้อมูลสถิติของจังหวัดและปีที่ระบุ
    ...
    // ใช้ AI อธิบาย record ที่ query ได้ ถ้าไม่มี OpenAI key จะ fallback เป็นข้อความสถิติ
    async ai(text) {
        const query = parseProvinceYear(text);
    ...
    // ส่งข้อความกลับไปยัง LINE ตามชนิด payload ที่ respond() สร้างไว้
...
export function validSignature(rawBody, signature, secret) {
    // LINE ใช้ HMAC-SHA256 กับ raw body จึงต้องตรวจจาก body ดิบก่อน parse JSON
