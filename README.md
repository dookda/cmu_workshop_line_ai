...
- LINE Messaging API, Text Message และ Flex Message
...
RAG ในโปรเจกต์นี้ตั้งใจใช้ lexical retrieval ที่อ่านง่ายในชั้นเรียน ข้อมูลจริงอยู่ที่ `backend/data/health_faq.json`; เปลี่ยนเป็น vector database หรือ embeddings ได้ภายหลังโดยคง interface `FAQRepository.search()` เดิม
...
| คำขอกราฟ เช่น `ขอกราฟฝุ่น PM2.5`, `กราฟไข้หวัดใหญ่รายเดือน` | OpenAI เลือกชุดข้อมูลที่ตรงที่สุดจาก `backend/data/health_stats.json` แล้วส่งกลับเป็นรูปภาพกราฟแท่งจริง (เรนเดอร์ผ่าน QuickChart) พร้อมแคปชันแหล่งข้อมูล (ถ้าไม่มี API key จะค้นหาในเครื่องแบบ lexical แทน) |
