...
15. [อ้างอิงฟังก์ชัน: คำอธิบายโค้ดทั้งหมด](#15-อ้างอิงฟังก์ชัน-คำอธิบายโค้ดทั้งหมด)
...
## 15. อ้างอิงฟังก์ชัน: คำอธิบายโค้ดทั้งหมด

หัวข้อนี้สรุปทุกฟังก์ชัน/คลาสที่มีอยู่จริงในโค้ดสุดท้าย (`backend/core.js`, `backend/routes.js`, `backend/server.js`, `backend/scripts/convert_province_stats.js`) เพื่อให้อ่านเข้าใจการทำงานได้รวดเร็ว โดยไม่ต้องไล่อ่านทั้งไฟล์

### `backend/core.js`

**`ProvinceStatsRepository`** — เก็บและค้นข้อมูลสถิติจังหวัด (โหลดจาก `province_stats.json` ครั้งเดียวตอนสร้าง instance)

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `constructor(path)` | อ่านไฟล์ JSON ที่ `path` แล้วเก็บไว้ใน `this.items`; คำนวณ `this.defaultYear` เป็นปีล่าสุดที่มีในข้อมูล เพื่อใช้เป็นค่า default เวลา query ไม่ระบุปี |
| `findByProvince(name, year)` | ค้นจังหวัดด้วยการเทียบ substring แบบไม่สนตัวพิมพ์เล็ก/ใหญ่ ถ้าระบุ `year` จะคืนระเบียนเดียวของปีนั้น ถ้าไม่ระบุจะคืน array ทุกปีที่มี (เรียงจากปีน้อยไปมาก) คืน `null` ถ้าไม่พบ |
| `query({ field, min, max, year })` | กรองข้อมูลของปีที่ระบุ (default = `defaultYear`) ตามช่วงค่า `min`–`max` ของฟิลด์ `field` (ต้องเป็นหนึ่งใน `patient`, `patient_rate`, `dead`, `dead_rate`, `cfr` มิฉะนั้น fallback เป็น `patient_rate`) แล้วเรียงจากมากไปน้อย |

**ฟังก์ชันช่วยจัดรูปแบบข้อความ**

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `textReply(text)` | ห่อ string ธรรมดาให้เป็น payload ข้อความสำหรับส่งกลับ LINE |
| `formatRecord(record)` | แปลงระเบียนเดียว (จังหวัด+ปีเดียว) เป็นข้อความสรุปผู้ป่วย/เสียชีวิต/CFR แบบอ่านง่าย |
| `formatRecords(records)` | แปลงหลายระเบียน (จังหวัดเดียว หลายปี) เป็นข้อความสรุปทีละบรรทัดต่อปี |

**ฟังก์ชันแยกวิเคราะห์คำสั่งจากผู้ใช้ (parsing)**

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `parseQuery(text)` | แยกข้อความ `"<จังหวัด> <ปี>"` ออกเป็น `{ province, year }`; ถ้าไม่มีเลขปีต่อท้าย คืน `year: undefined` |
| `matchPrefixed(text, keywords)` | helper กลาง ใช้ตรวจว่าข้อความเริ่มต้นด้วยคำใดคำหนึ่งใน `keywords` หรือไม่ ถ้าใช่คืนข้อความส่วนที่เหลือ (trim แล้ว) ถ้าไม่ใช่คืน `null` |
| `parseChartQuery(text)` | ใช้ `matchPrefixed` กับ `CHART_KEYWORDS` (เช่น "สรุปข้อมูล") เพื่อดึงชื่อจังหวัด และตัดเลขปีต่อท้ายทิ้งเสมอ เพราะกราฟจะพล็อตทุกปีอยู่แล้ว |
| `parseAiQuery(text)` | ใช้ `matchPrefixed` กับ `AI_KEYWORDS` (เช่น "ai") แล้วแยกต่อเป็น `{ province, year }`; คืน `null` ถ้าไม่มีเลขปีระบุมาด้วย (เพราะคำถาม AI ต้องระบุปีเสมอ) |

**การเชื่อม OpenAI**

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `aiExplain(record, { apiKey, model })` | ส่งข้อมูล 1 ระเบียนให้ OpenAI Responses API อธิบายเป็นภาษาที่เข้าใจง่าย (จำกัดด้วย `instructions` ให้ใช้เฉพาะข้อมูลที่ส่งให้ ห้ามแต่งตัวเลข/วินิจฉัย) ถ้าไม่มี `apiKey` หรือเรียก API ไม่สำเร็จ จะ fallback กลับไปใช้ `formatRecord(record)` แทนเสมอ |

**การสร้างกราฟ**

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `provinceChartImage(records)` | สร้าง Chart.js config แบบ bar chart จาก array ของระเบียน (แกน X = ปี, แกน Y = จำนวนผู้ป่วย) แล้วเข้ารหัสเป็น URL ของ QuickChart คืนเป็น payload รูปภาพ พร้อม caption สรุปช่วงปี |

**Flex Message builders**

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `flex(altText, contents)` | ห่อ `contents` (โครง bubble) ให้เป็น payload ประเภท flex message พร้อม `altText` สำหรับ notification |
| `actionRow(label, text, color)` | สร้างปุ่มแถวเดียว (กล่องสีพื้นหลัง + label + ไอคอน `›`) ที่เมื่อกดแล้วจะส่งข้อความ `text` เสมือนผู้ใช้พิมพ์เอง |
| `menuFlex()` | สร้างเมนูหลัก (bubble) มีปุ่ม "เลือกจังหวัด", "สรุปข้อมูล", "ถาม AI" |
| `listFlex(title, subtitle, rows)` | โครง bubble กลางสำหรับหน้ารายการแบบมีหัวข้อ+คำอธิบาย+ปุ่มหลายแถว ใช้เป็นฐานให้ `provinceListFlex` และ `yearListFlex` |
| `provinceListFlex()` | รายชื่อจังหวัดที่คัดสรรไว้ (`CURATED_PROVINCES`) ให้กดเลือก แต่ละปุ่มจะส่งคำสั่ง "เลือกปี <จังหวัด>" ต่อ |
| `yearListFlex(province)` | รายการปีให้เลือก (`YEAR_CHOICES`) ของจังหวัดที่ระบุ บวกปุ่ม "ทุกปี" ที่ส่งแค่ชื่อจังหวัดเฉยๆ (ไม่ระบุปี) |

**`LineService`** — ตัวกลางรับข้อความจากผู้ใช้ ตัดสินใจว่าต้องตอบอะไร แล้วส่งกลับผ่าน LINE Messaging API

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `constructor(accessToken, provinceStats, { apiKey, model })` | เก็บ token สำหรับเรียก LINE API, instance ของ `ProvinceStatsRepository`, และค่า config ของ OpenAI ไว้ใช้ภายหลัง |
| `respond(text)` | จุดตัดสินใจหลักของบอท ไล่ตรวจข้อความตามลำดับ: คำสั่งเมนู/help → คำสั่งกราฟ/เลือกจังหวัด/ai แบบไม่มีพารามิเตอร์ (ส่ง prompt อธิบายวิธีใช้) → คำสั่ง "ai ..." แบบมีพารามิเตอร์ (เรียก `aiExplain`) → คำสั่ง "เลือกปี ..." (เรียก `yearListFlex`) → คำสั่ง "สรุปข้อมูล ..." (เรียก `provinceChartImage`) → สุดท้ายตีความเป็นชื่อจังหวัด+ปี (เรียก `findByProvince` แล้ว format เป็นข้อความ) คืน payload ที่พร้อมส่งให้ `reply()` เสมอ |
| `reply(replyToken, payload)` | แปลง payload (`text` / `flex` / `image`) เป็นรูปแบบ message ของ LINE SDK แล้วเรียก `replyMessage`; ถ้าไม่มี `accessToken` จะข้ามการส่งจริงไปเลย (เผื่อกรณีทดสอบ/เดโมที่ยังไม่ได้ผูก token) |

**Signature validation**

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `validSignature(rawBody, signature, secret)` | คำนวณ HMAC-SHA256 ของ raw body ด้วย channel secret แล้วเทียบกับ header `x-line-signature` ที่ LINE ส่งมา โดยใช้ `timingSafeEqual` เพื่อป้องกัน timing attack คืน `true`/`false` |

### `backend/routes.js`

| ฟังก์ชัน/route | คำอธิบาย |
|---|---|
| `services()` | สร้าง `ProvinceStatsRepository` และ `LineService` แบบ lazy (ครั้งแรกที่มี request เข้ามาเท่านั้น) แล้วเก็บไว้ใน module-level cache (`_services`) เพื่อไม่ต้องอ่านไฟล์ JSON ซ้ำทุก request |
| `GET /health` | คืน `{ status: 'ok', service: 'healthline-ai' }` ใช้ตรวจว่า server ทำงานอยู่ |
| `sendProvince(res, provinceStats, name, year)` | helper เรียก `findByProvince` แล้วส่ง response กลับ: ถ้าพบส่งเป็น JSON ถ้าไม่พบส่ง `404` พร้อม `{ error: 'province not found' }` |
| `GET /api/provinces` | ถ้ามี query `province` จะมองหาจังหวัดนั้นโดยตรง (ผ่าน `sendProvince`) ถ้าไม่มีจะคืนรายการกรอง/เรียงตาม query `field`, `min`, `max`, `year` (ผ่าน `query()`) |
| `GET /api/provinces/:province` | ค้นจังหวัดเดียวจาก path param แทน query param รองรับ query `year` เสริม |
| `POST /webhook` | ตรวจ `x-line-signature` ด้วย `validSignature` ก่อนเสมอ (ปฏิเสธด้วย `400` ถ้าไม่ผ่าน) จากนั้น parse body เป็น JSON แล้ว loop ผ่านทุก event: ข้อความ text → `line.respond()` + `line.reply()`, follow event (เพิ่มเพื่อนใหม่) → ตอบเมนูอัตโนมัติ ครอบ loop ทั้งหมดด้วย try/catch เพื่อไม่ให้ error จาก event เดียวทำให้ process ล้ม แล้วตอบ `200 OK` กลับไปเสมอ |

### `backend/server.js`

ไม่มีฟังก์ชันแยก เป็นเพียงจุดเริ่มต้น (entrypoint) ของแอป: โหลด `.env`, ให้บริการไฟล์ static จาก `frontend/` ผ่าน `express.static`, ผูก `router` จาก `routes.js`, แล้วเปิด listen ที่ `PORT` (default `3000`)

### `backend/scripts/convert_province_stats.js`

สคริปต์รันครั้งเดียวสำหรับแปลง `assets/stat.xlsx` เป็น `backend/data/province_stats.json`

| ฟังก์ชัน | คำอธิบาย |
|---|---|
| `synthesizeRow(real, year)` | จากข้อมูลจริงของปี `REAL_YEAR` (2029) คำนวณประชากรประมาณคืนกลับจาก `patient_rate` แล้วสุ่มปรับ (`±30%`) จำนวนผู้ป่วยและผู้เสียชีวิตเพื่อสร้างข้อมูลย้อนหลังจำลองสำหรับปี `SYNTHETIC_YEARS` (2025–2028) ที่ไม่มีข้อมูลจริง พร้อมคำนวณ `patient_rate`, `dead_rate`, `cfr` ใหม่ให้สอดคล้องกัน |
