...
### Checkpoint 5: ตรวจ syntax หลังเริ่ม `core.js`
...
### Checkpoint 7: ตรวจ syntax หลังเพิ่ม helper
...
เปิดไฟล์ `api.http` แล้วเพิ่ม request นี้:

```http
### [Chart Manual] Create chart with QuickChart
  ...
  "chart": {
    ...
    "options": {
      "plugins": {
        "title": {
          "display": true,
          "text": "ตัวอย่างกราฟจังหวัดเชียงราย"
        },
...
ให้กด `Send Request` ที่ request นี้ หากสำเร็จ REST Client จะแสดงผลเป็นรูปกราฟ

ส่วนที่เชื่อมโยงกับโค้ดใน `chartImage` คือ:

- `type: 'bar'` กำหนดชนิดกราฟ
- `labels` คือ label แกน X
- `datasets[0].data` คือค่าที่ใช้วาดแท่งกราฟ
- `backgroundColor` คือสีของแท่งกราฟ
...
### Checkpoint 9: ทดสอบ Flex Message
...
### Checkpoint 10: ทดสอบ logic ของบอตโดยยังไม่จำเป็นต้องเชื่อมต่อ LINE
...
### Checkpoint 11: ตรวจ syntax หลังเพิ่ม signature
...
#### Checkpoint 13.2: ทดสอบ province API
...
#### Checkpoint 13.3: ทดสอบ LINE webhook
